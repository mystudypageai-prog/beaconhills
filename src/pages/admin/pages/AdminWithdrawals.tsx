...
export default function AdminWithdrawals() {
  ...
  const fetchData = useCallback(() => {
    ...
    api
      .get<PaginatedResponse<Withdrawal>>(`/admin/withdrawals?${params}`)
      .then((res) => {
  ...
  const reject = async (id: number) => {
    ...
    setMsg("");
    try {
      const res: any = await api.post(`/admin/withdrawals/${id}/reject`, {
        reason: rejectReason,
      });
      ...
      <div className={styles.statsGrid}>
        {[
          ...
          >
            ...
            <div style={{ fontSize: "20px", fontWeight: 700, color: s.clr }}>
              {s.value}
            </div>
          </div>
        ))}
        ...
        {loading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "1fr 120px 120px 180px 80px 100px 160px",
                gap: "12px",
                padding: "10px 16px",
                borderBottom: "1px solid rgba(255,255,255,0.06)",
                fontSize: "11px",
              ...
              return (
                <div
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns:
                      "1fr 120px 120px 180px 80px 100px 160px",
                    gap: "12px",
                    padding: "12px 16px",
                ...
                >
                  ...
                  <div style={{ fontSize: "13px", color: "#e5e7eb" }}>
                    {w.usd_amount ? formatUSD(Number(w.usd_amount)) : "-"}
                  </div>
                  <div style={{ fontSize: "12px", color: "#9ca3af" }}>
                    {w.crypto_amount
                      ? `${Number(w.crypto_amount).toFixed(6)} ${w.symbol || w.currency || ""}`
                      : "-"}
                  </div>
                  <div
                    style={{
                  ...
                  >
                    {w.recipient_address
                      ? `${w.recipient_address.slice(0, 14)}…${w.recipient_address.slice(-6)}`
                      : "-"}
                  </div>
                  <div style={{ fontSize: "12px", color: "#9ca3af" }}>
                    {w.network?.name || "-"}

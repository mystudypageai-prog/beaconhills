...
export default function AdminDeposits() {
  ...
  const fetchDeposits = useCallback(() => {
    ...
    api
      .get<PaginatedResponse<Deposit>>(`/admin/deposits?${params}`)
      .then((res) => {
  ...
  const reject = async (id: number) => {
    ...
    setMsg("");
    try {
      const res: any = await api.post(`/admin/deposits/${id}/reject`, {
        reason: rejectReason,
      });
  ...
  const totalConfirmedUsd = confirmed.reduce(
    (s, d) => s + Number(d.usd_amount || d.amount),
    0,
  );
      ...
      <div className={styles.statsGrid}>
        {[
          ...
          },
          {
            ...
            value: formatUSD(totalConfirmedUsd),
            clr: "#22c853",
            bg: "rgba(34,200,83,0.1)",
          },
          {
            label: "Pending",
          ...
          <div key={s.label} className={styles.statCard}>
            ...
            <div>
              ...
              <div className={styles.statValue} style={{ color: s.clr }}>
                {s.value}
              </div>
            </div>
          </div>
                ...
                <div key={dep.id} className={styles.tableRow}>
                  ...
                  <div>
                    <div style={{ fontSize: "13px", color: "#e5e7eb" }}>
                      {dep.usd_amount
                        ? formatUSD(Number(dep.usd_amount))
                        : `${Number(dep.amount).toFixed(6)} ${dep.currency}`}
                    </div>
                    {dep.usd_amount && (
                      <div style={{ fontSize: "11px", color: "#6b7280" }}>
                        {Number(dep.amount).toFixed(6)} {dep.currency}
                  ...
                  >
                    {dep.transaction_hash
                      ? `${dep.transaction_hash.slice(0, 12)}…`
                      : "-"}
                  </div>
                  <div>
                    <span

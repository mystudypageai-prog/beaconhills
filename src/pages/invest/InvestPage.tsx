...
const TradingViewWidget = memo(({ symbol }: TVWidgetProps) => {
  ...
  useEffect(() => {
    ...
    // Clear widget div
    const widgetDiv = container.current.querySelector(
...
function generateOrderBook(basePrice: number): {
  ...
  for (let i = 0; i < 8; i++) {
    const askPrice =
      basePrice +
      (i + 1) * (basePrice * 0.0002) +
      Math.random() * basePrice * 0.0001;
    const askAmt = +(Math.random() * 1.5 + 0.05).toFixed(4);
    asks.push({
      price: +askPrice.toFixed(2),
      amount: askAmt,
      total: +(askPrice * askAmt).toFixed(0),
    });
    ...
    bids.push({
      ...
      amount: bidAmt,
      total: +(bidPrice * bidAmt).toFixed(0),
    });
  }
  return { asks, bids };
}
...
function simulateNextPrice(
  ...
  const profitable = Math.random() < 0.9;
  ...
  return +(current + direction * magnitude + noise).toFixed(
    current < 10 ? 5 : 2,
  );
}
...
export default function InvestPage() {
  ...
  const [orderBook, setOrderBook] = useState(() =>
    generateOrderBook(COINS[0].price),
  );
  ...
  useEffect(() => {
    setLocalBalance(Number(user?.balance ?? 0));
  }, [user]);
    ...
    const interval = setInterval(() => {
      setCoinPrices((prev) => {
        ...
        COINS.forEach((c) => {
          ...
          next[c.symbol] = +(prev[c.symbol] + drift).toFixed(
            c.price < 10 ? 5 : 2,
          );
        });
        return next;
    ...
    const interval = setInterval(() => {
      setActiveTrades((prev) => {
        ...
        const remaining = prev
          .map((trade) => {
            const newPrice = simulateNextPrice(
              trade.currentPrice,
              trade.entryPrice,
              trade.side,
            );
            ...
            if (trade.tp && trade.side === "buy" && newPrice >= trade.tp)
              closeReason = "tp";
            if (trade.tp && trade.side === "sell" && newPrice <= trade.tp)
              closeReason = "tp";
            if (trade.sl && trade.side === "buy" && newPrice <= trade.sl)
              closeReason = "sl";
            if (trade.sl && trade.side === "sell" && newPrice >= trade.sl)
              closeReason = "sl";
            ...
            if (closeReason) {
              ...
              toClose.push({
                ...
                pnlPct: +((finalPnl / trade.usdValue) * 100).toFixed(2),
        ...
        if (toClose.length > 0) {
          ...
          setLocalBalance(
            (b) =>
              +(
                b +
                toClose.reduce((s, t) => s + t.usdValue, 0) +
                totalPnl
              ).toFixed(2),
          );
          // Persist to backend
  ...
  const handlePlaceOrder = useCallback(async () => {
    ...
    if (!qty || qty <= 0) {
      setPlaceError("Enter a valid amount");
      return;
    }
    ...
    if (orderSide === "buy" && usdCost + fee > localBalance) {
      setPlaceError("Insufficient balance");
      return;
    }
    ...
    try {
      ...
      setTpPrice("");
      setPlaceSuccess(
        `${orderSide === "buy" ? "Buy" : "Sell"} order placed - ${qty} ${selectedCoin.symbol} @ $${execPrice.toLocaleString()}`,
      );
      ...
      if (orderSide === "buy")
        setLocalBalance((b) => +(b + parseFloat(estTotal) * 1.001).toFixed(2));
    } finally {
      setIsPlacing(false);
      setTimeout(() => {
        setPlaceSuccess("");
  ...
  const handlePercentage = (pct: number) => {
    ...
    const demoHolding =
      activeTrades
        .filter(
          (t) => t.pair === `${selectedCoin.symbol}/USD` && t.side === "sell",
        )
            ...
            <div className={styles.ordersTabRow}>
              ...
              >
                ...
                {activeTrades.length > 0 && (
                  <span className={styles.badge}>{activeTrades.length}</span>
                )}
              </button>
              <button
              ...
              <div className={styles.ordersTable}>
                ...
                {activeTrades.length === 0 ? (
                  <div className={styles.emptyOrders}>
                    No active trades. Place an order to get started.
                  </div>
                ) : (
                  activeTrades.map((t) => (
                    <div
                      key={t.id}
                      className={`${styles.ordersRow} ${styles.activeRow}`}
                    >
                      <span className={styles.pairTag}>{t.pair}</span>
                      <span
                        className={
                          t.side === "buy" ? styles.buyText : styles.sellText
                        }
                      >
                        {t.side.toUpperCase()}
                      </span>
                      ...
                      <span className={styles.slText}>
                        {t.sl ? `$${t.sl.toLocaleString()}` : "-"}
                      </span>
                      <span className={styles.tpText}>
                        {t.tp ? `$${t.tp.toLocaleString()}` : "-"}
                      </span>
                      <span
                        className={t.pnl >= 0 ? styles.pnlPos : styles.pnlNeg}
                      >
              ...
              <div className={styles.ordersTable}>
                ...
                {closedTrades.length === 0 ? (
                  ...
                  closedTrades.map((t) => (
                    <div
                      key={t.id}
                      className={`${styles.ordersRow} ${styles.historyRow}`}
                    >
                      <span className={styles.pairTag}>{t.pair}</span>
                      <span
                        className={
          ...
          <div className={styles.formCard}>
            ...
            <div className={styles.feeRow}>
              ...
              <span className={styles.feeVal}>
                ${(parseFloat(estTotal) * 0.001 || 0).toFixed(2)}
              </span>
            </div>

            {/* Feedback */}
            {placeError && (
              <div className={styles.feedbackError}>{placeError}</div>
            )}
            {placeSuccess && (
              <div className={styles.feedbackSuccess}>{placeSuccess}</div>
            )}
